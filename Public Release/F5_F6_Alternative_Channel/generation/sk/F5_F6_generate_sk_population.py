...
Comparing FM550 vs Control, 2x sk vs Control, and 3x sk vs Control
...
def modify_sk_conductance(cell, scale_factor=1.0, compartment='all'):
    """Modify sk conductance by a scale factor."""
    for section in cell.all:
        if _should_modify_section(section, compartment):
            if hasattr(section, 'gbar_sk'):
                for seg in section:
                    seg.gbar_sk *= scale_factor
...
def calculate_rectification_fast(cell_type='dmsn', cell_index=0, v_init=-82.8, 
                                sk_modification=None, use_female_params=False):
    """
    ...
    try:
        ...
        # Apply sk modifications once
        if sk_modification is not None and 'scale_factor' in sk_modification:
            modify_sk_conductance(cell, sk_modification['scale_factor'])
...
def run_population_analysis(cell_types=['dmsn', 'imsn'], cell_indices=None, sample_size=None):
    ...
    results_data = {
        ...
        'sk125x_holding_current': [],
        'sk15x_holding_current': [],
        'fm550_vs_control_percent': [],
        'sk125x_vs_control_percent': [],
        'sk15x_vs_control_percent': []
    }
        ...
        for i, cell_idx in enumerate(cells_to_analyze):
            ...
            try:
                ...
                fm550_ir, fm550_holding_current = calculate_rectification_fast(
                    ...
                    sk_modification=None, use_female_params=True
                )
                print("✓")
                ...
                # Condition 3: 1.25x sk
                print(f"  Running 1.25x sk condition...", end=" ")
                sk125x_ir, sk125x_holding_current = calculate_rectification_fast(
                    cell_type=cell_type, cell_index=cell_idx, v_init=-82.8,
                    sk_modification={'scale_factor': 1.25}, use_female_params=False
                )
                print("✓")
                
                if sk125x_ir is None:
                    print(f"  ❌ Failed to calculate 1.25x sk for {cell_type} cell {cell_idx}")
                    continue
                
                # Condition 4: 1.5x sk
                print(f"  Running 1.5x sk condition...", end=" ")
                sk15x_ir, sk15x_holding_current = calculate_rectification_fast(
                    cell_type=cell_type, cell_index=cell_idx, v_init=-82.8,
                    sk_modification={'scale_factor': 1.5}, use_female_params=False
                )
                print("✓")
                
                if sk15x_ir is None:
                    print(f"  ❌ Failed to calculate 1.5x sk for {cell_type} cell {cell_idx}")
                    continue
                
                # Calculate percent differences
                fm550_percent = ((fm550_ir - control_ir) / control_ir) * 100
                sk125x_percent = ((sk125x_ir - control_ir) / control_ir) * 100
                sk15x_percent = ((sk15x_ir - control_ir) / control_ir) * 100
                ...
                results_data['sk125x_ir'].append(sk125x_ir)
                results_data['sk15x_ir'].append(sk15x_ir)
                ...
                results_data['sk125x_holding_current'].append(sk125x_holding_current)
                results_data['sk15x_holding_current'].append(sk15x_holding_current)
                results_data['fm550_vs_control_percent'].append(fm550_percent)
                results_data['sk125x_vs_control_percent'].append(sk125x_percent)
                results_data['sk15x_vs_control_percent'].append(sk15x_percent)
                ...
                print(f"    1.25x sk: {sk125x_ir:.2f} MΩ ({sk125x_percent:+.1f}%) | 1.5x sk: {sk15x_ir:.2f} MΩ ({sk15x_percent:+.1f}%)")
...
def create_population_plots(df):
    """Create comprehensive population plots for both DMSN and IMSN with 1.25x and 1.5x sk."""
    ...
    for cell_type, sub_df in [('DMSN', dmsn_df), ('IMSN', imsn_df)]:
        if len(sub_df) > 0:
            ...
            print(f"  1.25x sk vs Control:")
            print(f"    Mean ± SEM: {sub_df['sk125x_vs_control_percent'].mean():.1f} ± {sub_df['sk125x_vs_control_percent'].sem():.1f}%")
            print(f"    Range: {sub_df['sk125x_vs_control_percent'].min():.1f}% to {sub_df['sk125x_vs_control_percent'].max():.1f}%")
            print(f"  1.5x sk vs Control:")
            print(f"    Mean ± SEM: {sub_df['sk15x_vs_control_percent'].mean():.1f} ± {sub_df['sk15x_vs_control_percent'].sem():.1f}%")
            print(f"    Range: {sub_df['sk15x_vs_control_percent'].min():.1f}% to {sub_df['sk15x_vs_control_percent'].max():.1f}%")
    ...
    for cell_type, sub_df in [('DMSN', dmsn_df), ('IMSN', imsn_df)]:
        if len(sub_df) > 1:
            ...
            sk125x_tstat, sk125x_pval = stats.ttest_1samp(sub_df['sk125x_vs_control_percent'], 0)
            sk15x_tstat, sk15x_pval = stats.ttest_1samp(sub_df['sk15x_vs_control_percent'], 0)
            
            print(f"  FM550 vs Control: t({len(sub_df)-1}) = {fm550_tstat:.3f}, p = {fm550_pval:.6f}")
            print(f"  1.25x sk vs Control: t({len(sub_df)-1}) = {sk125x_tstat:.3f}, p = {sk125x_pval:.6f}")
            print(f"  1.5x sk vs Control: t({len(sub_df)-1}) = {sk15x_tstat:.3f}, p = {sk15x_pval:.6f}")
            
            # Paired t-tests between conditions
            fm550_sk125x_tstat, fm550_sk125x_pval = stats.ttest_rel(sub_df['fm550_vs_control_percent'], sub_df['sk125x_vs_control_percent'])
            fm550_sk15x_tstat, fm550_sk15x_pval = stats.ttest_rel(sub_df['fm550_vs_control_percent'], sub_df['sk15x_vs_control_percent'])
            sk125x_sk15x_tstat, sk125x_sk15x_pval = stats.ttest_rel(sub_df['sk125x_vs_control_percent'], sub_df['sk15x_vs_control_percent'])
            
            print(f"  FM550 vs 1.25x sk: t({len(sub_df)-1}) = {fm550_sk125x_tstat:.3f}, p = {fm550_sk125x_pval:.6f}")
            print(f"  FM550 vs 1.5x sk: t({len(sub_df)-1}) = {fm550_sk15x_tstat:.3f}, p = {fm550_sk15x_pval:.6f}")
            print(f"  1.25x sk vs 1.5x sk: t({len(sub_df)-1}) = {sk125x_sk15x_tstat:.3f}, p = {sk125x_sk15x_pval:.6f}")
    ...
    if len(dmsn_df) > 0 and len(imsn_df) > 0:
        ...
        sk125x_tstat, sk125x_pval = stats.ttest_ind(dmsn_df['sk125x_vs_control_percent'], imsn_df['sk125x_vs_control_percent'])
        sk15x_tstat, sk15x_pval = stats.ttest_ind(dmsn_df['sk15x_vs_control_percent'], imsn_df['sk15x_vs_control_percent'])
        
        print(f"  FM550 (DMSN vs IMSN): t = {fm550_tstat:.3f}, p = {fm550_pval:.6f}")
        print(f"  1.25x sk (DMSN vs IMSN): t = {sk125x_tstat:.3f}, p = {sk125x_pval:.6f}")
        print(f"  1.5x sk (DMSN vs IMSN): t = {sk15x_tstat:.3f}, p = {sk15x_pval:.6f}")
    ...
    dmsn_sk125x_color = '#DC143C'    # Crimson
    dmsn_sk15x_color = '#FF6B6B'     # Light red
    imsn_fm550_color = '#00008B'      # Dark blue
    imsn_sk125x_color = '#1E90FF'    # Dodger blue
    imsn_sk15x_color = '#87CEEB'     # Sky blue
    ...
    if len(dmsn_df) > 0:
        ...
        ax_combined.scatter(x_dmsn, dmsn_df['sk125x_vs_control_percent'], 
                          color=dmsn_sk125x_color, alpha=0.8, s=60, marker='o', 
                          edgecolors='black', linewidth=1,
                          label=f'DMSN 1.25x sk vs Control (n={len(dmsn_df)})')
        ax_combined.scatter(x_dmsn, dmsn_df['sk15x_vs_control_percent'], 
                          color=dmsn_sk15x_color, alpha=0.8, s=60, marker='o', 
                          edgecolors='black', linewidth=1,
                          label=f'DMSN 1.5x sk vs Control (n={len(dmsn_df)})')
        
        # Connect paired points
        for i in range(len(dmsn_df)):
            ax_combined.plot([x_dmsn[i], x_dmsn[i]], 
                           [dmsn_df['fm550_vs_control_percent'].iloc[i], 
                            dmsn_df['sk125x_vs_control_percent'].iloc[i]], 
                           color='red', alpha=0.2, linewidth=1)
            ax_combined.plot([x_dmsn[i], x_dmsn[i]], 
                           [dmsn_df['sk125x_vs_control_percent'].iloc[i], 
                            dmsn_df['sk15x_vs_control_percent'].iloc[i]], 
                           color='red', alpha=0.2, linewidth=1)
    ...
    if len(imsn_df) > 0:
        ...
        ax_combined.scatter(x_imsn, imsn_df['sk125x_vs_control_percent'], 
                          color=imsn_sk125x_color, alpha=0.8, s=60, marker='o', 
                          edgecolors='black', linewidth=1,
                          label=f'IMSN 1.25x sk vs Control (n={len(imsn_df)})')
        ax_combined.scatter(x_imsn, imsn_df['sk15x_vs_control_percent'], 
                          color=imsn_sk15x_color, alpha=0.8, s=60, marker='o', 
                          edgecolors='black', linewidth=1,
                          label=f'IMSN 1.5x sk vs Control (n={len(imsn_df)})')
        
        # Connect paired points
        for i in range(len(imsn_df)):
            ax_combined.plot([x_imsn[i], x_imsn[i]], 
                           [imsn_df['fm550_vs_control_percent'].iloc[i], 
                            imsn_df['sk125x_vs_control_percent'].iloc[i]], 
                           color='green', alpha=0.2, linewidth=1)
            ax_combined.plot([x_imsn[i], x_imsn[i]], 
                           [imsn_df['sk125x_vs_control_percent'].iloc[i], 
                            imsn_df['sk15x_vs_control_percent'].iloc[i]], 
                           color='green', alpha=0.2, linewidth=1)
    ...
    if len(dmsn_df) > 0:
        ...
        dmsn_sk125x_mean = dmsn_df['sk125x_vs_control_percent'].mean()
        dmsn_sk15x_mean = dmsn_df['sk15x_vs_control_percent'].mean()
        ax_combined.axhline(dmsn_fm550_mean, color=dmsn_fm550_color, linestyle='-', alpha=0.7, linewidth=3)
        ax_combined.axhline(dmsn_sk125x_mean, color=dmsn_sk125x_color, linestyle='-', alpha=0.7, linewidth=3)
        ax_combined.axhline(dmsn_sk15x_mean, color=dmsn_sk15x_color, linestyle='-', alpha=0.7, linewidth=3)
    
    if len(imsn_df) > 0:
        imsn_fm550_mean = imsn_df['fm550_vs_control_percent'].mean()
        imsn_sk125x_mean = imsn_df['sk125x_vs_control_percent'].mean()
        imsn_sk15x_mean = imsn_df['sk15x_vs_control_percent'].mean()
        ax_combined.axhline(imsn_fm550_mean, color=imsn_fm550_color, linestyle='-', alpha=0.7, linewidth=3)
        ax_combined.axhline(imsn_sk125x_mean, color=imsn_sk125x_color, linestyle='-', alpha=0.7, linewidth=3)
        ax_combined.axhline(imsn_sk15x_mean, color=imsn_sk15x_color, linestyle='-', alpha=0.7, linewidth=3)
    ...
    ax_combined.set_title(f'Population Analysis: FM550 vs 1.25x sk vs 1.5x sk Effects\n'
    ...
    cell_type_data = [('DMSN', dmsn_df, dmsn_fm550_color, dmsn_sk125x_color, dmsn_sk15x_color), 
                      ('IMSN', imsn_df, imsn_fm550_color, imsn_sk125x_color, imsn_sk15x_color)]
    
    for idx, (cell_type, sub_df, fm550_color, sk125x_color, sk15x_color) in enumerate(cell_type_data):
        if len(sub_df) == 0:
            continue
        ...
        ax.scatter(x_positions, sub_df['sk125x_vs_control_percent'], 
                  color=sk125x_color, alpha=0.8, s=70, marker='o', 
                  edgecolors='black', linewidth=1,
                  label=f'1.25x sk vs Control')
        ax.scatter(x_positions, sub_df['sk15x_vs_control_percent'], 
                  color=sk15x_color, alpha=0.8, s=70, marker='o', 
                  edgecolors='black', linewidth=1,
                  label=f'1.5x sk vs Control')
        
        # Connect paired points
        for i in range(len(sub_df)):
            ax.plot([x_positions[i], x_positions[i]], 
                   [sub_df['fm550_vs_control_percent'].iloc[i], 
                    sub_df['sk125x_vs_control_percent'].iloc[i]], 
                   color='gray', alpha=0.3, linewidth=1)
            ax.plot([x_positions[i], x_positions[i]], 
                   [sub_df['sk125x_vs_control_percent'].iloc[i], 
                    sub_df['sk15x_vs_control_percent'].iloc[i]], 
                   color='gray', alpha=0.3, linewidth=1)
        ...
        sk125x_mean = sub_df['sk125x_vs_control_percent'].mean()
        sk15x_mean = sub_df['sk15x_vs_control_percent'].mean()
        ax.axhline(fm550_mean, color=fm550_color, linestyle='-', alpha=0.8, linewidth=3)
        ax.axhline(sk125x_mean, color=sk125x_color, linestyle='-', alpha=0.8, linewidth=3)
        ax.axhline(sk15x_mean, color=sk15x_color, linestyle='-', alpha=0.8, linewidth=3)
        ...
        sk125x_sem = sub_df['sk125x_vs_control_percent'].sem()
        sk15x_sem = sub_df['sk15x_vs_control_percent'].sem()
        
        textstr = f'FM550: {fm550_mean:.1f} ± {fm550_sem:.1f}%\n' \
                  f'1.25x sk: {sk125x_mean:.1f} ± {sk125x_sem:.1f}%\n' \
                  f'1.5x sk: {sk15x_mean:.1f} ± {sk15x_sem:.1f}%'
...
if __name__ == '__main__':
    print("=" * 80)
    print("MSN POPULATION ANALYSIS: FM550 vs 1.25x sk vs 1.5x sk")
    ...
    print("  - Conditions: Control, FM550, 1.25x sk, 1.5x sk")
