...
Comparing FM550 vs Control, 2x kaf vs Control, and 3x kaf vs Control
...
def modify_kaf_conductance(cell, scale_factor=1.0, compartment='all'):
    """Modify kaf conductance by a scale factor."""
    for section in cell.all:
        if _should_modify_section(section, compartment):
            if hasattr(section, 'gbar_kaf'):
                for seg in section:
                    seg.gbar_kaf *= scale_factor
...
def calculate_rectification_fast(cell_type='dmsn', cell_index=0, v_init=-82.8, 
                                kaf_modification=None, use_female_params=False):
    """
    ...
    try:
        ...
        # Apply kaf modifications once
        if kaf_modification is not None and 'scale_factor' in kaf_modification:
            modify_kaf_conductance(cell, kaf_modification['scale_factor'])
...
def run_population_analysis(cell_types=['dmsn', 'imsn'], cell_indices=None, sample_size=None):
    ...
    results_data = {
        ...
        'kaf125x_holding_current': [],
        'kaf15x_holding_current': [],
        'fm550_vs_control_percent': [],
        'kaf125x_vs_control_percent': [],
        'kaf15x_vs_control_percent': []
    }
        ...
        for i, cell_idx in enumerate(cells_to_analyze):
            ...
            try:
                ...
                fm550_ir, fm550_holding_current = calculate_rectification_fast(
                    ...
                    kaf_modification=None, use_female_params=True
                )
                print("✓")
                ...
                # Condition 3: 1.25x kaf
                print(f"  Running 1.25x kaf condition...", end=" ")
                kaf125x_ir, kaf125x_holding_current = calculate_rectification_fast(
                    cell_type=cell_type, cell_index=cell_idx, v_init=-82.8,
                    kaf_modification={'scale_factor': 1.25}, use_female_params=False
                )
                print("✓")
                
                if kaf125x_ir is None:
                    print(f"  ❌ Failed to calculate 1.25x kaf for {cell_type} cell {cell_idx}")
                    continue
                
                # Condition 4: 1.5x kaf
                print(f"  Running 1.5x kaf condition...", end=" ")
                kaf15x_ir, kaf15x_holding_current = calculate_rectification_fast(
                    cell_type=cell_type, cell_index=cell_idx, v_init=-82.8,
                    kaf_modification={'scale_factor': 1.5}, use_female_params=False
                )
                print("✓")
                
                if kaf15x_ir is None:
                    print(f"  ❌ Failed to calculate 1.5x kaf for {cell_type} cell {cell_idx}")
                    continue
                
                # Calculate percent differences
                fm550_percent = ((fm550_ir - control_ir) / control_ir) * 100
                kaf125x_percent = ((kaf125x_ir - control_ir) / control_ir) * 100
                kaf15x_percent = ((kaf15x_ir - control_ir) / control_ir) * 100
                ...
                results_data['kaf125x_ir'].append(kaf125x_ir)
                results_data['kaf15x_ir'].append(kaf15x_ir)
                ...
                results_data['kaf125x_holding_current'].append(kaf125x_holding_current)
                results_data['kaf15x_holding_current'].append(kaf15x_holding_current)
                results_data['fm550_vs_control_percent'].append(fm550_percent)
                results_data['kaf125x_vs_control_percent'].append(kaf125x_percent)
                results_data['kaf15x_vs_control_percent'].append(kaf15x_percent)
                ...
                print(f"    1.25x kaf: {kaf125x_ir:.2f} MΩ ({kaf125x_percent:+.1f}%) | 1.5x kaf: {kaf15x_ir:.2f} MΩ ({kaf15x_percent:+.1f}%)")
...
def create_population_plots(df):
    """Create comprehensive population plots for both DMSN and IMSN with 1.25x and 1.5x kaf."""
    ...
    for cell_type, sub_df in [('DMSN', dmsn_df), ('IMSN', imsn_df)]:
        if len(sub_df) > 0:
            ...
            print(f"  1.25x kaf vs Control:")
            print(f"    Mean ± SEM: {sub_df['kaf125x_vs_control_percent'].mean():.1f} ± {sub_df['kaf125x_vs_control_percent'].sem():.1f}%")
            print(f"    Range: {sub_df['kaf125x_vs_control_percent'].min():.1f}% to {sub_df['kaf125x_vs_control_percent'].max():.1f}%")
            print(f"  1.5x kaf vs Control:")
            print(f"    Mean ± SEM: {sub_df['kaf15x_vs_control_percent'].mean():.1f} ± {sub_df['kaf15x_vs_control_percent'].sem():.1f}%")
            print(f"    Range: {sub_df['kaf15x_vs_control_percent'].min():.1f}% to {sub_df['kaf15x_vs_control_percent'].max():.1f}%")
    ...
    for cell_type, sub_df in [('DMSN', dmsn_df), ('IMSN', imsn_df)]:
        if len(sub_df) > 1:
            ...
            kaf125x_tstat, kaf125x_pval = stats.ttest_1samp(sub_df['kaf125x_vs_control_percent'], 0)
            kaf15x_tstat, kaf15x_pval = stats.ttest_1samp(sub_df['kaf15x_vs_control_percent'], 0)
            
            print(f"  FM550 vs Control: t({len(sub_df)-1}) = {fm550_tstat:.3f}, p = {fm550_pval:.6f}")
            print(f"  1.25x kaf vs Control: t({len(sub_df)-1}) = {kaf125x_tstat:.3f}, p = {kaf125x_pval:.6f}")
            print(f"  1.5x kaf vs Control: t({len(sub_df)-1}) = {kaf15x_tstat:.3f}, p = {kaf15x_pval:.6f}")
            
            # Paired t-tests between conditions
            fm550_kaf125x_tstat, fm550_kaf125x_pval = stats.ttest_rel(sub_df['fm550_vs_control_percent'], sub_df['kaf125x_vs_control_percent'])
            fm550_kaf15x_tstat, fm550_kaf15x_pval = stats.ttest_rel(sub_df['fm550_vs_control_percent'], sub_df['kaf15x_vs_control_percent'])
            kaf125x_kaf15x_tstat, kaf125x_kaf15x_pval = stats.ttest_rel(sub_df['kaf125x_vs_control_percent'], sub_df['kaf15x_vs_control_percent'])
            
            print(f"  FM550 vs 1.25x kaf: t({len(sub_df)-1}) = {fm550_kaf125x_tstat:.3f}, p = {fm550_kaf125x_pval:.6f}")
            print(f"  FM550 vs 1.5x kaf: t({len(sub_df)-1}) = {fm550_kaf15x_tstat:.3f}, p = {fm550_kaf15x_pval:.6f}")
            print(f"  1.25x kaf vs 1.5x kaf: t({len(sub_df)-1}) = {kaf125x_kaf15x_tstat:.3f}, p = {kaf125x_kaf15x_pval:.6f}")
    ...
    if len(dmsn_df) > 0 and len(imsn_df) > 0:
        ...
        kaf125x_tstat, kaf125x_pval = stats.ttest_ind(dmsn_df['kaf125x_vs_control_percent'], imsn_df['kaf125x_vs_control_percent'])
        kaf15x_tstat, kaf15x_pval = stats.ttest_ind(dmsn_df['kaf15x_vs_control_percent'], imsn_df['kaf15x_vs_control_percent'])
        
        print(f"  FM550 (DMSN vs IMSN): t = {fm550_tstat:.3f}, p = {fm550_pval:.6f}")
        print(f"  1.25x kaf (DMSN vs IMSN): t = {kaf125x_tstat:.3f}, p = {kaf125x_pval:.6f}")
        print(f"  1.5x kaf (DMSN vs IMSN): t = {kaf15x_tstat:.3f}, p = {kaf15x_pval:.6f}")
    ...
    dmsn_kaf125x_color = '#DC143C'    # Crimson
    dmsn_kaf15x_color = '#FF6B6B'     # Light red
    imsn_fm550_color = '#00008B'      # Dark blue
    imsn_kaf125x_color = '#1E90FF'    # Dodger blue
    imsn_kaf15x_color = '#87CEEB'     # Sky blue
    ...
    if len(dmsn_df) > 0:
        ...
        ax_combined.scatter(x_dmsn, dmsn_df['kaf125x_vs_control_percent'], 
                          color=dmsn_kaf125x_color, alpha=0.8, s=60, marker='o', 
                          edgecolors='black', linewidth=1,
                          label=f'DMSN 1.25x kaf vs Control (n={len(dmsn_df)})')
        ax_combined.scatter(x_dmsn, dmsn_df['kaf15x_vs_control_percent'], 
                          color=dmsn_kaf15x_color, alpha=0.8, s=60, marker='o', 
                          edgecolors='black', linewidth=1,
                          label=f'DMSN 1.5x kaf vs Control (n={len(dmsn_df)})')
        
        # Connect paired points
        for i in range(len(dmsn_df)):
            ax_combined.plot([x_dmsn[i], x_dmsn[i]], 
                           [dmsn_df['fm550_vs_control_percent'].iloc[i], 
                            dmsn_df['kaf125x_vs_control_percent'].iloc[i]], 
                           color='red', alpha=0.2, linewidth=1)
            ax_combined.plot([x_dmsn[i], x_dmsn[i]], 
                           [dmsn_df['kaf125x_vs_control_percent'].iloc[i], 
                            dmsn_df['kaf15x_vs_control_percent'].iloc[i]], 
                           color='red', alpha=0.2, linewidth=1)
    ...
    if len(imsn_df) > 0:
        ...
        ax_combined.scatter(x_imsn, imsn_df['kaf125x_vs_control_percent'], 
                          color=imsn_kaf125x_color, alpha=0.8, s=60, marker='o', 
                          edgecolors='black', linewidth=1,
                          label=f'IMSN 1.25x kaf vs Control (n={len(imsn_df)})')
        ax_combined.scatter(x_imsn, imsn_df['kaf15x_vs_control_percent'], 
                          color=imsn_kaf15x_color, alpha=0.8, s=60, marker='o', 
                          edgecolors='black', linewidth=1,
                          label=f'IMSN 1.5x kaf vs Control (n={len(imsn_df)})')
        
        # Connect paired points
        for i in range(len(imsn_df)):
            ax_combined.plot([x_imsn[i], x_imsn[i]], 
                           [imsn_df['fm550_vs_control_percent'].iloc[i], 
                            imsn_df['kaf125x_vs_control_percent'].iloc[i]], 
                           color='green', alpha=0.2, linewidth=1)
            ax_combined.plot([x_imsn[i], x_imsn[i]], 
                           [imsn_df['kaf125x_vs_control_percent'].iloc[i], 
                            imsn_df['kaf15x_vs_control_percent'].iloc[i]], 
                           color='green', alpha=0.2, linewidth=1)
    ...
    if len(dmsn_df) > 0:
        ...
        dmsn_kaf125x_mean = dmsn_df['kaf125x_vs_control_percent'].mean()
        dmsn_kaf15x_mean = dmsn_df['kaf15x_vs_control_percent'].mean()
        ax_combined.axhline(dmsn_fm550_mean, color=dmsn_fm550_color, linestyle='-', alpha=0.7, linewidth=3)
        ax_combined.axhline(dmsn_kaf125x_mean, color=dmsn_kaf125x_color, linestyle='-', alpha=0.7, linewidth=3)
        ax_combined.axhline(dmsn_kaf15x_mean, color=dmsn_kaf15x_color, linestyle='-', alpha=0.7, linewidth=3)
    
    if len(imsn_df) > 0:
        imsn_fm550_mean = imsn_df['fm550_vs_control_percent'].mean()
        imsn_kaf125x_mean = imsn_df['kaf125x_vs_control_percent'].mean()
        imsn_kaf15x_mean = imsn_df['kaf15x_vs_control_percent'].mean()
        ax_combined.axhline(imsn_fm550_mean, color=imsn_fm550_color, linestyle='-', alpha=0.7, linewidth=3)
        ax_combined.axhline(imsn_kaf125x_mean, color=imsn_kaf125x_color, linestyle='-', alpha=0.7, linewidth=3)
        ax_combined.axhline(imsn_kaf15x_mean, color=imsn_kaf15x_color, linestyle='-', alpha=0.7, linewidth=3)
    ...
    ax_combined.set_title(f'Population Analysis: FM550 vs 1.25x kaf vs 1.5x kaf Effects\n'
    ...
    cell_type_data = [('DMSN', dmsn_df, dmsn_fm550_color, dmsn_kaf125x_color, dmsn_kaf15x_color), 
                      ('IMSN', imsn_df, imsn_fm550_color, imsn_kaf125x_color, imsn_kaf15x_color)]
    
    for idx, (cell_type, sub_df, fm550_color, kaf125x_color, kaf15x_color) in enumerate(cell_type_data):
        if len(sub_df) == 0:
            continue
        ...
        ax.scatter(x_positions, sub_df['kaf125x_vs_control_percent'], 
                  color=kaf125x_color, alpha=0.8, s=70, marker='o', 
                  edgecolors='black', linewidth=1,
                  label=f'1.25x kaf vs Control')
        ax.scatter(x_positions, sub_df['kaf15x_vs_control_percent'], 
                  color=kaf15x_color, alpha=0.8, s=70, marker='o', 
                  edgecolors='black', linewidth=1,
                  label=f'1.5x kaf vs Control')
        
        # Connect paired points
        for i in range(len(sub_df)):
            ax.plot([x_positions[i], x_positions[i]], 
                   [sub_df['fm550_vs_control_percent'].iloc[i], 
                    sub_df['kaf125x_vs_control_percent'].iloc[i]], 
                   color='gray', alpha=0.3, linewidth=1)
            ax.plot([x_positions[i], x_positions[i]], 
                   [sub_df['kaf125x_vs_control_percent'].iloc[i], 
                    sub_df['kaf15x_vs_control_percent'].iloc[i]], 
                   color='gray', alpha=0.3, linewidth=1)
        ...
        kaf125x_mean = sub_df['kaf125x_vs_control_percent'].mean()
        kaf15x_mean = sub_df['kaf15x_vs_control_percent'].mean()
        ax.axhline(fm550_mean, color=fm550_color, linestyle='-', alpha=0.8, linewidth=3)
        ax.axhline(kaf125x_mean, color=kaf125x_color, linestyle='-', alpha=0.8, linewidth=3)
        ax.axhline(kaf15x_mean, color=kaf15x_color, linestyle='-', alpha=0.8, linewidth=3)
        ...
        kaf125x_sem = sub_df['kaf125x_vs_control_percent'].sem()
        kaf15x_sem = sub_df['kaf15x_vs_control_percent'].sem()
        
        textstr = f'FM550: {fm550_mean:.1f} ± {fm550_sem:.1f}%\n' \
                  f'1.25x kaf: {kaf125x_mean:.1f} ± {kaf125x_sem:.1f}%\n' \
                  f'1.5x kaf: {kaf15x_mean:.1f} ± {kaf15x_sem:.1f}%'
...
if __name__ == '__main__':
    print("=" * 80)
    print("MSN POPULATION ANALYSIS: FM550 vs 1.25x kaf vs 1.5x kaf")
    ...
    print("  - Conditions: Control, FM550, 1.25x kaf, 1.5x kaf")
