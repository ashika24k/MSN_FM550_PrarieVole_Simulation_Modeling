...
Comparing FM550 vs Control, 2x bk vs Control, and 3x bk vs Control
...
def modify_bk_conductance(cell, scale_factor=1.0, compartment='all'):
    """Modify bk conductance bk a scale factor."""
    for section in cell.all:
        if _should_modify_section(section, compartment):
            if hasattr(section, 'gbar_bk'):
                for seg in section:
                    seg.gbar_bk *= scale_factor
...
def calculate_rectification_fast(cell_type='dmsn', cell_index=0, v_init=-82.8, 
                                bk_modification=None, use_female_params=False):
    """
    ...
    try:
        ...
        # Apply bk modifications once
        if bk_modification is not None and 'scale_factor' in bk_modification:
            modify_bk_conductance(cell, bk_modification['scale_factor'])
        ...
        for step_idx, i_test in enumerate(neg_currents):
            ...
            # Use searchsorted for fast indexing (faster than mabking)
...
def run_population_analysis(cell_types=['dmsn', 'imsn'], cell_indices=None, sample_size=None):
    ...
    results_data = {
        ...
        'bk125x_holding_current': [],
        'bk15x_holding_current': [],
        'fm550_vs_control_percent': [],
        'bk125x_vs_control_percent': [],
        'bk15x_vs_control_percent': []
    }
        ...
        for i, cell_idx in enumerate(cells_to_analyze):
            ...
            try:
                ...
                fm550_ir, fm550_holding_current = calculate_rectification_fast(
                    ...
                    bk_modification=None, use_female_params=True
                )
                print("✓")
                ...
                # Condition 3: 1.25x bk
                print(f"  Running 1.25x bk condition...", end=" ")
                bk125x_ir, bk125x_holding_current = calculate_rectification_fast(
                    cell_type=cell_type, cell_index=cell_idx, v_init=-82.8,
                    bk_modification={'scale_factor': 1.25}, use_female_params=False
                )
                print("✓")
                
                if bk125x_ir is None:
                    print(f"  ❌ Failed to calculate 1.25x bk for {cell_type} cell {cell_idx}")
                    continue
                
                # Condition 4: 1.5x bk
                print(f"  Running 1.5x bk condition...", end=" ")
                bk15x_ir, bk15x_holding_current = calculate_rectification_fast(
                    cell_type=cell_type, cell_index=cell_idx, v_init=-82.8,
                    bk_modification={'scale_factor': 1.5}, use_female_params=False
                )
                print("✓")
                
                if bk15x_ir is None:
                    print(f"  ❌ Failed to calculate 1.5x bk for {cell_type} cell {cell_idx}")
                    continue
                
                # Calculate percent differences
                fm550_percent = ((fm550_ir - control_ir) / control_ir) * 100
                bk125x_percent = ((bk125x_ir - control_ir) / control_ir) * 100
                bk15x_percent = ((bk15x_ir - control_ir) / control_ir) * 100
                ...
                results_data['bk125x_ir'].append(bk125x_ir)
                results_data['bk15x_ir'].append(bk15x_ir)
                ...
                results_data['bk125x_holding_current'].append(bk125x_holding_current)
                results_data['bk15x_holding_current'].append(bk15x_holding_current)
                results_data['fm550_vs_control_percent'].append(fm550_percent)
                results_data['bk125x_vs_control_percent'].append(bk125x_percent)
                results_data['bk15x_vs_control_percent'].append(bk15x_percent)
                ...
                print(f"    1.25x bk: {bk125x_ir:.2f} MΩ ({bk125x_percent:+.1f}%) | 1.5x bk: {bk15x_ir:.2f} MΩ ({bk15x_percent:+.1f}%)")
...
def create_population_plots(df):
    """Create comprehensive population plots for both DMSN and IMSN with 1.25x and 1.5x bk."""
    
    # Separate data bk cell type
    ...
    for cell_type, sub_df in [('DMSN', dmsn_df), ('IMSN', imsn_df)]:
        if len(sub_df) > 0:
            ...
            print(f"  1.25x bk vs Control:")
            print(f"    Mean ± SEM: {sub_df['bk125x_vs_control_percent'].mean():.1f} ± {sub_df['bk125x_vs_control_percent'].sem():.1f}%")
            print(f"    Range: {sub_df['bk125x_vs_control_percent'].min():.1f}% to {sub_df['bk125x_vs_control_percent'].max():.1f}%")
            print(f"  1.5x bk vs Control:")
            print(f"    Mean ± SEM: {sub_df['bk15x_vs_control_percent'].mean():.1f} ± {sub_df['bk15x_vs_control_percent'].sem():.1f}%")
            print(f"    Range: {sub_df['bk15x_vs_control_percent'].min():.1f}% to {sub_df['bk15x_vs_control_percent'].max():.1f}%")
    ...
    for cell_type, sub_df in [('DMSN', dmsn_df), ('IMSN', imsn_df)]:
        if len(sub_df) > 1:
            ...
            bk125x_tstat, bk125x_pval = stats.ttest_1samp(sub_df['bk125x_vs_control_percent'], 0)
            bk15x_tstat, bk15x_pval = stats.ttest_1samp(sub_df['bk15x_vs_control_percent'], 0)
            
            print(f"  FM550 vs Control: t({len(sub_df)-1}) = {fm550_tstat:.3f}, p = {fm550_pval:.6f}")
            print(f"  1.25x bk vs Control: t({len(sub_df)-1}) = {bk125x_tstat:.3f}, p = {bk125x_pval:.6f}")
            print(f"  1.5x bk vs Control: t({len(sub_df)-1}) = {bk15x_tstat:.3f}, p = {bk15x_pval:.6f}")
            
            # Paired t-tests between conditions
            fm550_bk125x_tstat, fm550_bk125x_pval = stats.ttest_rel(sub_df['fm550_vs_control_percent'], sub_df['bk125x_vs_control_percent'])
            fm550_bk15x_tstat, fm550_bk15x_pval = stats.ttest_rel(sub_df['fm550_vs_control_percent'], sub_df['bk15x_vs_control_percent'])
            bk125x_bk15x_tstat, bk125x_bk15x_pval = stats.ttest_rel(sub_df['bk125x_vs_control_percent'], sub_df['bk15x_vs_control_percent'])
            
            print(f"  FM550 vs 1.25x bk: t({len(sub_df)-1}) = {fm550_bk125x_tstat:.3f}, p = {fm550_bk125x_pval:.6f}")
            print(f"  FM550 vs 1.5x bk: t({len(sub_df)-1}) = {fm550_bk15x_tstat:.3f}, p = {fm550_bk15x_pval:.6f}")
            print(f"  1.25x bk vs 1.5x bk: t({len(sub_df)-1}) = {bk125x_bk15x_tstat:.3f}, p = {bk125x_bk15x_pval:.6f}")
    ...
    if len(dmsn_df) > 0 and len(imsn_df) > 0:
        ...
        bk125x_tstat, bk125x_pval = stats.ttest_ind(dmsn_df['bk125x_vs_control_percent'], imsn_df['bk125x_vs_control_percent'])
        bk15x_tstat, bk15x_pval = stats.ttest_ind(dmsn_df['bk15x_vs_control_percent'], imsn_df['bk15x_vs_control_percent'])
        
        print(f"  FM550 (DMSN vs IMSN): t = {fm550_tstat:.3f}, p = {fm550_pval:.6f}")
        print(f"  1.25x bk (DMSN vs IMSN): t = {bk125x_tstat:.3f}, p = {bk125x_pval:.6f}")
        print(f"  1.5x bk (DMSN vs IMSN): t = {bk15x_tstat:.3f}, p = {bk15x_pval:.6f}")
    ...
    dmsn_bk125x_color = '#DC143C'    # Crimson
    dmsn_bk15x_color = '#FF6B6B'     # Light red
    imsn_fm550_color = '#00008B'      # Dark blue
    imsn_bk125x_color = '#1E90FF'    # Dodger blue
    imsn_bk15x_color = '#87CEEB'     # bky blue
    ...
    if len(dmsn_df) > 0:
        ...
        ax_combined.scatter(x_dmsn, dmsn_df['bk125x_vs_control_percent'], 
                          color=dmsn_bk125x_color, alpha=0.8, s=60, marker='o', 
                          edgecolors='black', linewidth=1,
                          label=f'DMSN 1.25x bk vs Control (n={len(dmsn_df)})')
        ax_combined.scatter(x_dmsn, dmsn_df['bk15x_vs_control_percent'], 
                          color=dmsn_bk15x_color, alpha=0.8, s=60, marker='o', 
                          edgecolors='black', linewidth=1,
                          label=f'DMSN 1.5x bk vs Control (n={len(dmsn_df)})')
        
        # Connect paired points
        for i in range(len(dmsn_df)):
            ax_combined.plot([x_dmsn[i], x_dmsn[i]], 
                           [dmsn_df['fm550_vs_control_percent'].iloc[i], 
                            dmsn_df['bk125x_vs_control_percent'].iloc[i]], 
                           color='red', alpha=0.2, linewidth=1)
            ax_combined.plot([x_dmsn[i], x_dmsn[i]], 
                           [dmsn_df['bk125x_vs_control_percent'].iloc[i], 
                            dmsn_df['bk15x_vs_control_percent'].iloc[i]], 
                           color='red', alpha=0.2, linewidth=1)
    ...
    if len(imsn_df) > 0:
        ...
        ax_combined.scatter(x_imsn, imsn_df['bk125x_vs_control_percent'], 
                          color=imsn_bk125x_color, alpha=0.8, s=60, marker='o', 
                          edgecolors='black', linewidth=1,
                          label=f'IMSN 1.25x bk vs Control (n={len(imsn_df)})')
        ax_combined.scatter(x_imsn, imsn_df['bk15x_vs_control_percent'], 
                          color=imsn_bk15x_color, alpha=0.8, s=60, marker='o', 
                          edgecolors='black', linewidth=1,
                          label=f'IMSN 1.5x bk vs Control (n={len(imsn_df)})')
        
        # Connect paired points
        for i in range(len(imsn_df)):
            ax_combined.plot([x_imsn[i], x_imsn[i]], 
                           [imsn_df['fm550_vs_control_percent'].iloc[i], 
                            imsn_df['bk125x_vs_control_percent'].iloc[i]], 
                           color='green', alpha=0.2, linewidth=1)
            ax_combined.plot([x_imsn[i], x_imsn[i]], 
                           [imsn_df['bk125x_vs_control_percent'].iloc[i], 
                            imsn_df['bk15x_vs_control_percent'].iloc[i]], 
                           color='green', alpha=0.2, linewidth=1)
    ...
    if len(dmsn_df) > 0:
        ...
        dmsn_bk125x_mean = dmsn_df['bk125x_vs_control_percent'].mean()
        dmsn_bk15x_mean = dmsn_df['bk15x_vs_control_percent'].mean()
        ax_combined.axhline(dmsn_fm550_mean, color=dmsn_fm550_color, linestyle='-', alpha=0.7, linewidth=3)
        ax_combined.axhline(dmsn_bk125x_mean, color=dmsn_bk125x_color, linestyle='-', alpha=0.7, linewidth=3)
        ax_combined.axhline(dmsn_bk15x_mean, color=dmsn_bk15x_color, linestyle='-', alpha=0.7, linewidth=3)
    
    if len(imsn_df) > 0:
        imsn_fm550_mean = imsn_df['fm550_vs_control_percent'].mean()
        imsn_bk125x_mean = imsn_df['bk125x_vs_control_percent'].mean()
        imsn_bk15x_mean = imsn_df['bk15x_vs_control_percent'].mean()
        ax_combined.axhline(imsn_fm550_mean, color=imsn_fm550_color, linestyle='-', alpha=0.7, linewidth=3)
        ax_combined.axhline(imsn_bk125x_mean, color=imsn_bk125x_color, linestyle='-', alpha=0.7, linewidth=3)
        ax_combined.axhline(imsn_bk15x_mean, color=imsn_bk15x_color, linestyle='-', alpha=0.7, linewidth=3)
    ...
    ax_combined.set_title(f'Population Analysis: FM550 vs 1.25x bk vs 1.5x bk Effects\n'
    ...
    cell_type_data = [('DMSN', dmsn_df, dmsn_fm550_color, dmsn_bk125x_color, dmsn_bk15x_color), 
                      ('IMSN', imsn_df, imsn_fm550_color, imsn_bk125x_color, imsn_bk15x_color)]
    
    for idx, (cell_type, sub_df, fm550_color, bk125x_color, bk15x_color) in enumerate(cell_type_data):
        if len(sub_df) == 0:
            continue
        ...
        ax.scatter(x_positions, sub_df['bk125x_vs_control_percent'], 
                  color=bk125x_color, alpha=0.8, s=70, marker='o', 
                  edgecolors='black', linewidth=1,
                  label=f'1.25x bk vs Control')
        ax.scatter(x_positions, sub_df['bk15x_vs_control_percent'], 
                  color=bk15x_color, alpha=0.8, s=70, marker='o', 
                  edgecolors='black', linewidth=1,
                  label=f'1.5x bk vs Control')
        
        # Connect paired points
        for i in range(len(sub_df)):
            ax.plot([x_positions[i], x_positions[i]], 
                   [sub_df['fm550_vs_control_percent'].iloc[i], 
                    sub_df['bk125x_vs_control_percent'].iloc[i]], 
                   color='gray', alpha=0.3, linewidth=1)
            ax.plot([x_positions[i], x_positions[i]], 
                   [sub_df['bk125x_vs_control_percent'].iloc[i], 
                    sub_df['bk15x_vs_control_percent'].iloc[i]], 
                   color='gray', alpha=0.3, linewidth=1)
        ...
        bk125x_mean = sub_df['bk125x_vs_control_percent'].mean()
        bk15x_mean = sub_df['bk15x_vs_control_percent'].mean()
        ax.axhline(fm550_mean, color=fm550_color, linestyle='-', alpha=0.8, linewidth=3)
        ax.axhline(bk125x_mean, color=bk125x_color, linestyle='-', alpha=0.8, linewidth=3)
        ax.axhline(bk15x_mean, color=bk15x_color, linestyle='-', alpha=0.8, linewidth=3)
        ...
        bk125x_sem = sub_df['bk125x_vs_control_percent'].sem()
        bk15x_sem = sub_df['bk15x_vs_control_percent'].sem()
        
        textstr = f'FM550: {fm550_mean:.1f} ± {fm550_sem:.1f}%\n' \
                  f'1.25x bk: {bk125x_mean:.1f} ± {bk125x_sem:.1f}%\n' \
                  f'1.5x bk: {bk15x_mean:.1f} ± {bk15x_sem:.1f}%'
...
if __name__ == '__main__':
    print("=" * 80)
    print("MSN POPULATION ANALYSIS: FM550 vs 1.25x bk vs 1.5x bk")
    ...
    print("  - Conditions: Control, FM550, 1.25x bk, 1.5x bk")
