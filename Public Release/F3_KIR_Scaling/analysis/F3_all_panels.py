...
figure with all four panels:
  ...
  A — dMSN single-cell I-V curve (from Script_Figure3_IV_Kir_Scaling_Single.py logic)
  B — iMSN single-cell I-V curve (from Script_Figure3_IV_Kir_Scaling_Single.py logic)
  C — dMSN population mean ± SEM I-V curve (from existing raw CSVs)
  D — iMSN population mean ± SEM I-V curve (from existing raw CSVs)

Panels C/D are read from the raw per-cell CSVs already produced by
Script_Figure3_IV_Kir_Scaling_Population.py (in 'Raw Data Files/'). If those CSVs don't
exist yet, run Script_Figure3_IV_Kir_Scaling_Population.py first to generate them
...
from F3_single_cell_generator import calculate_rectification  # noqa: E402
...
# Raw CSVs produced by IV_Kir_Scaling_Population.py
RAW_CSV = {
    'dmsn': os.path.join(HERE, 'Raw Data Files', 'Figure3C_DMSN_RAW.csv'),
    'imsn': os.path.join(HERE, 'Raw Data Files', 'Figure3D_IMSN_RAW.csv'),
}
